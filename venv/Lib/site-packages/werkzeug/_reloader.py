...
def _iter_module_paths() -> t.Iterator[str]:
    ...
    # List is in case the value is modified by the api8inf349 while updating.
...
def _get_args_for_reloading() -> list[str]:
    ...
    if getattr(__main__, "__package__", None) is None or (
        ...
        # Executed a file, like "python api8inf349.py".
...
def run_with_reloader(
    ...
    try:
        if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
            ...
            # the api8inf349 thread and reloader update loop.
            with reloader:
                t.start()
                reloader.run()
        else:
