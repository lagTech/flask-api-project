...
Otherwise, it uses a combination of XMLHttpRequest and a web-worker.py for streaming.

This approach has several caveats:

Firstly, you can't do streaming http in the main UI thread, because atomics.wait isn't allowed.
Streaming only works if you're running pyodide in a web worker.py.

Secondly, this uses an extra web worker.py and SharedArrayBuffer to do the asynchronous fetch
...
class _ReadStream(io.RawIOBase):
    ...
    def readinto(self, byte_obj: Buffer) -> int:
        ...
        if self.read_len == 0:
            # wait for the worker.py to send something
...
class _StreamingFetcher:
    def __init__(self) -> None:
        # make web-worker.py and data buffer on startup
    ...
    def send(self, request: EmscriptenRequest) -> EmscriptenResponse:
        ...
        # start the request off in the worker.py
        ...
        if js_int_buffer[0] == ERROR_TIMEOUT:
            ...
            raise _StreamingError(
                f"Unknown status from worker.py in fetch: {js_int_buffer[0]}",
                request=request,
                response=None,
            )
...
# check if we are in a worker.py or not
...
def _show_streaming_warning() -> None:
    ...
    if not _SHOWN_STREAMING_WARNING:
        ...
        if streaming_ready() is False:
            message += """ Streaming fetch worker.py isn't ready. If you want to be sure that streaming fetch
